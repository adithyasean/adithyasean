## Hi there! 👋 I'm **Adithya Séan Bandara Ekanayaka**

Welcome to my GitHub profile! I'm a **computer science undergraduate** at NIBM Colombo with interests in **AI**, **robotics**, **machine learning**, and **software engineering**. I love solving real-world problems through technology.

---

## 👨‍💻 What I Do:
- **AI & Machine Learning**: Building intelligent solutions, including NLP, computer vision, and generative AI.
- **Robotics and IoT**: Developing low-cost autonomous systems and embedded solutions.
- **Cloud Computing**: Using Oracle Cloud, Google Cloud, AWS, and Azure for deployment.
- **Web Development**: Creating scalable applications with Node.js, React, Spring Boot, and FastAPI.
- **Full-Stack Development**: Integrating front-end and back-end technologies for complete solutions.

---

## 📚 Education:
- **BSc (Hons) Computing** (Awarded by Coventry University) - National Institute of Business Management (NIBM), Colombo (2024 - Current)
- **Higher National Diploma in Software Engineering** - NIBM, Colombo (2023)
- **Diploma in Software Engineering** - NIBM, Colombo (2022)

---

## 💼 Work Experience:
- **Software Engineer** - Araaya Business Solutions, Colombo (Aug 2025 - Current)
- **Intern Software Engineer** - Sri Lanka Telecom, Colombo (Feb 2025 - Aug 2025)
  - Developed a real-time face recognition attendance system with hybrid attendance marking capabilities.

---

## 🏆 Achievements & Certifications:
- **Rootcode Tech-Triathlon 2025 Finalist** - Selected as a finalist among top 8 teams (Apr 2025 - Sep 2025)
- **Commercial Bank International Hackathon Finalist** - Top 8 out of 30 teams, developed Singlish chatbot using LLaMA 3.1 8B (May 2024 - Current)
- **Oracle Cloud Infrastructure 2024 Generative AI Certified Professional** (Jul 2024)
- **Linux Foundations Introduction to Linux** (Sep 2025)
- **Google Developer Program** - Active participant (Jun 2024 - Current)
- **IEEE Student Member** (2024)

---

## 📚 Projects:
- **ComBank sinGPT Hackathon Project**: A Singlish chatbot using LLaMA 3.1 8B and LoRA. (Top 8 finalist)
- **Realtime Attendance Marking System for Sri Lanka Telecom**: Real-time face recognition system using FastAPI, Python, C++, React.
- **Next Step: AI-Powered Career Guidance**: Web app with microservices (Spring Boot), ML models, Docker, Kubernetes.
- **Enhanced Vision for Security Cameras**: Improved face detection in low-light using OpenCV.
- **Human-Following Robot**: Autonomous robot using repurposed smartphone and OpenBot project.
- **School Management System**: Modern system for Sri Lankan government schools.

---

## 🚀 Skills:
- **Programming**: Python, Java, C++, C#, JavaScript, CSS, HTML, PHP, SQL
- **Web Development**: Node.js, React, Flask, FastAPI, Spring Boot, Express.js
- **AI/ML Frameworks**: PyTorch, TensorFlow, OpenCV, Scikit-learn, Pandas, NumPy
- **Cloud Platforms**: Oracle Cloud, Google Cloud, AWS, Azure
- **DevOps**: Docker, Kubernetes, CI/CD
- **Databases**: SQL, Oracle DB, PostgreSQL, MongoDB, Firebase, MySQL, SQLite
- **Robotics & IoT**: Arduino, Raspberry Pi, ROS, FreeRTOS, ESP32, Microcontrollers (PIC)
- **Tools**: Git, GitHub, GitLab, Jupyter, Figma
- **Testing**: PyTest, Jest
- **Concepts**: Microservices, Generative AI, Machine Learning, Computer Networking, OOP, Data Structures & Algorithms, Digital Image Processing, Embedded Systems

---

## 🌐 Languages:
- **English**: Proficient (C1 level)
- **Sinhala**: Mother tongue

---

## 🎯 Hobbies & Interests:
- Chess
- Table Tennis
- Machine Learning projects

---

## 📫 Let's Connect:
- **Email**: adithyasean@gmail.com
- **LinkedIn**: [linkedin.com/in/adithyasean](https://www.linkedin.com/in/adithyasean/)
- **GitHub**: [github.com/adithyasean](https://github.com/adithyasean)
- **Website**: [adithyasean.github.io](https://adithyasean.github.io)
- **Hugging Face**: [huggingface.co/adithyasean](https://huggingface.co/adithyasean)
- **Credly**: [credly.com/users/adithyasean](https://www.credly.com/users/adithyasean)
- **Phone/WhatsApp**: (+94) 702967909

Feel free to check out my repositories and connect for collaborations!
